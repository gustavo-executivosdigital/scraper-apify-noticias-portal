Searches **Google Search results** for any term, picks the strongest stories with AI, rewrites each one as an **original article** ready to republish, and generates an **editorial image** for it — all in one run.

## What does Notícias Portal Rewriter do?

Give it a search term like `taxas de logística` or `zé felipe e ana maria`. The Actor finds recent Google results, uses a **Groq AI** to select the best ones (you choose how many), and rewrites each selected story in fresh, original wording based on the available title, snippet, source, and date. Finally it generates a matching **news image** with **Google Gemini 2.5 Flash Image (Nano Banana)** via **OpenRouter**.
...
| --- | --- | --- |
| `searchQuery` | Term to search on Google. **Required.** | — |
| `maxArticles` | How many Google results to evaluate. | `10` |
...
Cost now comes from one composed Actor — `apify/google-search-scraper` for discovery — plus this Actor's compute. The AI calls run on **your own Groq and OpenRouter keys**, so the text and image generation add no Apify platform cost. Fetch fewer articles (`maxArticles`) to reduce Compute Units.

## Tips

- Lower `maxArticles` to spend less on Google Search discovery.
...
- **Why was a story skipped?** The result may be a duplicate, a non-article page, or too weak for the AI selection step.
